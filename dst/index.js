...
export class RenderBuffer {
    ...
    resize(width, height) {
        ...
        for (let i = 0; i < width * height; i++)
            this.data[i] = new RenderBuffer.Data();
        this.clear();
    }
    clear() {
    ...
    drawTriangles(vertices, indices, vertexShader, pixelShader, doDepthTesting = true) {
    ...
    #drawTriangle(v1, v2, v3, pixelShader, doDepthTesting = true) {
        ...
        for (let py = 0, y = -1; py < this.height; py++, y += dy) {
            for (let px = 0, x = -1; px < this.width; px++, x += dx, i++) {
                ...
                // depth testing
                const depth = 1 / iz;
                if (doDepthTesting && this.data[i].depth > depth)
                    continue;
                // clip near & far
                if (iz < -1 || iz > 1)
                    continue;
                ...
                const pixel = new Array(pix1.length);
...
(function (RenderBuffer) {
    class Data {
        pixel;
        depth;
        constructor(pixel = new PixelData(0, 1, 1, 1, 0), depth = 0) {
            this.pixel = pixel;
            this.depth = depth;
        }
    }
